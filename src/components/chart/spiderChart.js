...
export default function SpiderChart ({
    ...
    const angles = Array.from(Array(keys.length).keys()).map(
        i => Math.PI / 2 + (2 * Math.PI * i) / keys.length,
    );

    const interpolateFunc = d3.piecewise(ticks);
    const positionFunc = v => bisect(interpolateFunc, v);
    const linear = d3.scaleLinear().range([0, radius * ratio * 0.9]);
