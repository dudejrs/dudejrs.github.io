...
export default function ({data, type, axis, width, height, margin, children, color = "#565656",
		pointRadius = 2.5
	}){
	...
	return (
		<svg width={width} height={height}>
			<path fill="none" stroke={color} stroke-width="1.5" d={line(refineUndefined(data))} />
			<g fill={color} stroke= {color} stroke-width="1.5">
				{refineUndefined(data).map((d,i) => (<circle key={i} cx={x(axis[i])} cy={y(d)} r={pointRadius}/>))}
			</g>
			{children}
		</svg>
		);
