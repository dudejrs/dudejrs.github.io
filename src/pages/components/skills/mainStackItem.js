...
function generateStackItems(items, levels, colors){
	...
	if (typeof items[0] == "object" )
		return items.map((subStack)=>(<SubStackItem key={subStack.title} title={subStack.title} items={subStack.items} levels={subStack.levels} colors={colors} />));

	return <div></div>
}

export default function MainStackItem({children, title, items, levels, colors}) {
