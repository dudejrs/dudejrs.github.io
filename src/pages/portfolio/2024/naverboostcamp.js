...
function ListItem({title, tags}) {
	return (<li className={`${styles.listItem}`}>
			 ...
			 {
			 	tags && tags.map(t => <span className={`${styles.number}`}>{t}</span>)
			 }
			 </div>
			 </li>)
}
		...
		<Printable filename='2024_네이버부스트캠프_포트폴리오' layout='landscape'>		
			...
			<Page className={`${styles.container}`}>
				...
				<div className={`${styles.sections}`}>
					...
					<div className={`${styles.section} ${styles.between}`}>
						<List title={`배운점`}> 
							<ListItem title={`Angular의 바인딩 문법과 디렉티브`} tags={[1]}/>
							<ListItem title={`Angular의 주요 개념 : 양방향 바인딩, 계층적 인젝터, 변경감지 전략`} tags={[1]}/>
			...
			<Page className={`${styles.container}`}>
				...
				{Object.keys(data) != 0 && data["Infra"] && <VerticalTimeLine data={data["Infra"]["DBMS,SQL,Linux,Docker,Kubernetices"]} width={500} height={500} mapper={planMapper} ratio={1.5} ratios={[0, 0.5, 2, 0]} bmargin={50} stroke={2} radius={4} color={'#aaa'} minSize={[1000, 500]} className={`${styles.timeline}`}/>} 
					<div className={`${styles.section}`}>
						<List title={`배운점`}> 
							<ListItem title={`Oracle DBMS 및 MySQL 서버의 아키텍쳐 구조`} tags={[]} />
							<ListItem title={`트랜잭션 수준과 동시성 제어 방법`} tags={[]} />
							<ListItem title={`쿼리 힌트 작성 방법 및 SQL 실행 계획 읽는 방법`} tags={[]} />
							<ListItem title={`SQL 성능 튜닝 원리 : 라이브러리 캐시 최적화, 데이터베이스 Call 최소화, I/O 효율화`} tags={[]} />
							<ListItem title={`리눅스 디렉토리 구조 및 패키지 관리`} tags={[]} />
							<ListItem title={`리눅스 주요 셸 커맨드`} tags={[]} />
							<ListItem title={`리눅스의 주요 서비스: DNS, Apache, Proftp, Samba, NFS`} tags={[]} />
						</List>
						<List title={`네이버 부스트 캠프에 활용할 수 있는 점`}>
							<ListItem title={`데이터베이스 설계: 테이블 구조 선택 및 인덱스 설계`} tags={[]} />
							<ListItem title={`간단한 쿼리 성능 튜닝 및 트러블 슈팅`} tags={[]} />
							<ListItem title={`DBMS 설정 : 트랜잭션 레벨 설정, 버퍼캐시  라이브러리 캐시 크기 설정`} tags={[]} />
							<ListItem title={`xietd 및 crontab을 통한 서비스 데몬 및 배치프로그램 설정`} tags={[]} />
							<ListItem title={`리눅스 서버의 네트워크 관련 설정`} tags={[]} />
						</List>
					</div>
				</div>
			...
			<Page className={`${styles.container}`}>
				...
				<div className={`${styles.sections}`}>
					...
					<div className={`${styles.section}`}>
						<List title={`배운점`}> 
							<ListItem title={`도메인 설계의 주요 개념 : ENTITY, 값 객체(VO), 서비스, Aggregate, 팩터리, 레포지터리, 바운디드 컨텍스트`} tags={[1, 5]} />
							<ListItem title={`계층 구조 아키텍처와 각 계층의 역할 : 표현, 응용, 도메인, 인프라 스트럭쳐`} tags={[1, 5]} />
							<ListItem title={`명세기반 테스트, 구조적 테스트, 속성 기반 테스트, 테스트 주도 개발`} tags={[2]} />
							<ListItem title={`계약에 의한 코드 설계 : 사전조건, 사후조건, 불변식`} tags={[2, 3, 4]} />
							<ListItem title={`코드 품질의 핵심 요소 : 가독성, 예측가능성, 재사용성, 일반화성, 테스트 용이성`} tags={[3]} />
							<ListItem title={`명시적, 암시적 오류 전달 방법`} tags={[3]} />
							<ListItem title={`객체 불변성을 유지하는 방법`} tags={[3]} />
							<ListItem title={`테스트 더블의 종류 : Mock, Stub, Fake`} tags={[2,3]} />
							<ListItem title={`코드 재사용을 위한 두가지 방법 : 상속과 합성`} tags={[4]} />
							<ListItem title={`책임주도적 객체 지향 설계 방법 : GRASP`} tags={[4, 7]} />
							<ListItem title={`명령-쿼리 분리 및 참조 투명성, 불변성`} tags={[4]} />
							<ListItem title={`프로그래밍 패러다임 : 구조적, 객체지향적, 함수형 프로그래밍`} tags={[6]} />
							<ListItem title={`SOLID원칙 : SRP, OCP, LSP, ISP, DIP`} tags={[6]} />
							<ListItem title={`컴포넌트 응집도 원칙(REP, CRP, CRP)과 결합도 원칙(ADP, SDP, SAP)`} tags={[6]} />
							<ListItem title={`소프트웨어 아키텍쳐의 개념: 정책, 세부사항, 저수준/고수준, 경계`} tags={[6]} />
							<ListItem title={`문제 해결의 두가지 관점 : 구조적 관점과 기능적 관점`} tags={[6, 7]} />
