...
async function fetchCategories(client, categories) {
	...
	for (let category of categories) {
		const myJob = new DatabaseJob({
			...
			file : `${process.env.project_path}/public/test/categories/${category}.json`,
			exec : fetchCategory,
			finish : save,
		})
	...
	writeMetaData(`${process.env.project_path}/public/test/categories`)
}
