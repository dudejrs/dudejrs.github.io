...
async function fetchLog({client, path}) {
    const end = new Date(Date.now() + 24 * 60 * 60 * 1000)
        .toISOString()
        .slice(0, 10);
    const begin = new Date(Date.now() - 365 * 24 * 60 * 60 * 1000)
        .toISOString()
        .slice(0, 10);

    let response = await client.queryDatabase(
        process.env.NOTION_CODING_PRACTICE_LOG_DATABASE_ID,
        ANDFilter.of(
            DateFilter.Before('Date', end),
            DateFilter.After('Date', begin),
        ).build(),
    );
    ...
    for await (let {results} of response) {
        for (let page of results) {
            const {date, ...d} = await CodingPracticeLog.convert(page, {
                client,
            });
            ret[date.start] = d;
        }
