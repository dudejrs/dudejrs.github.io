...
async function getInfra(client) {
	...
	const data = await PlanTagsScheme.convert([],{tags : ["DBMS", "SQL", "Linux", "Docker", "Kubernetices"], client})
...
async function getData({client, path}) {
	...
	writeFileSync(`${path}/plan.json`, JSON.stringify(ret), {encoding : 'utf-8', flag : 'w+'})
}
