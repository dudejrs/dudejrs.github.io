...
async function refinePlan(result, categories, secret) {
    ...
    output['남은 시간'] = '0';
    output['Tag'] = result['properties']['Tag']['multi_select'].map(
        item => item['name'],
    );
    output['완료'] = result['properties']['완료']['checkbox']
        ? 'true'
        : 'false';
    output['장기/단기'] = result['properties']['장기/단기']['multi_select'].map(
        item => item['name'],
    );

    console.log(result['properties']);
    output['완료율'] = await getProperty(
        result['id'],
        result['properties']['완료율']['id'],
        secret,
    ).then(({data}) => {
...
function saveResult(result, dirPath) {
    ...
    fs.writeFile(file_path, JSON.stringify(result), err => {
        console.log(err);
    });
}
...
function _saveDetailPlans(detailedPlanList, dirPath) {
    ...
    fs.writeFile(file_path, JSON.stringify(uniqueDetailedPlanList), err => {
        console.log(err);
    });
}
