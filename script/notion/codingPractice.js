...
async function getCodingPracticeAggregation(
    notion,
    langauges,
    dirPath,
    secret,
    ...
    let aggregationByCategries = await getAggregationByCategories(
        notion,
        langauges,
        secret,
    );
    let aggregationByProblemType = await getAggregationByProblemType(
        notion,
        langauges,
    );

    fs.writeFileSync(
        `${dirPath}/totalProblem.json`,
        JSON.stringify(totalProblem),
        err => {
            console.log(err);
        },
    );
    ...
    fs.writeFileSync(
        `${dirPath}/aggregationByCategries.json`,
        JSON.stringify(aggregationByCategries),
        err => {
            console.log(err);
        },
    );
    ...
    fs.writeFileSync(
        `${dirPath}/aggregationByProblemType.json`,
        JSON.stringify(aggregationByProblemType),
        err => {
            console.log(err);
        },
    );
...
async function getTotalProblems(notion, langauges) {
    ...
    while (has_more_) {
        ...
        repetition += results.reduce(
            (acc, result) => acc + result['properties']['Repetition']['number'],
            0,
        );
    }
...
async function refineAggregation(result, langauges, secret) {
    ...
    const count = await getProperty(pageId, countID, secret).then(
        ({data}) => data['formula']['number'],
    );
    const repetition = await getProperty(pageId, repetitioniId, secret).then(
        ({data}) => data['formula']['number'],
    );
    ...
    for (let i = 0; i < langauges.length; i++) {
        ...
        r[langauge] = await getProperty(pageId, langaugeId, secret).then(
            ({data}) => data['formula']['number'],
        );
    }
...
async function getAggregationByProblemType(notion, langauges) {
    ...
    while (has_more_) {
        ...
        results.forEach(result =>
            aggregateResult(result, countByTypes, langauges),
        );
    }
...
function aggregateResult(result, countByTypes, langauges) {
    const problemTypes = result['properties']['문제유형']['multi_select'].map(
        m => m['name'],
    );
...
function logging(dirPath, langauges) {
    ...
    if (fs.existsSync(`${dirPath}/totalProblem.json`)) {
        let totalProblem = JSON.parse(
            fs.readFileSync(`${dirPath}/totalProblem.json`),
        );
    ...
    if (fs.existsSync(`${dirPath}/aggregationByCategries.json`)) {
        let aggregationByCategries = JSON.parse(
            fs.readFileSync(`${dirPath}/aggregationByCategries.json`),
        );
