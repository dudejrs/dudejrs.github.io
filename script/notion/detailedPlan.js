...
async function refineData(data, secret) {
    ...
    output['날짜'] = await getProperty(
        data['id'],
        data['properties']['날짜']['id'],
        secret,
    ).then(({data}) => {
        return data['results']['formula'];
    });
    output['Date'] = data['properties']['Date']['date'];
    output['진행상태'] = data['properties']['진행상태']['select']
        ? data['properties']['진행상태']['select']['name']
        : null;
    output['세부계획'] =
        data['properties']['세부계획']['title'][0]['plain_text'];

    return output;
}

async function getDetailedPlan(id, secret) {
    return await getPage(id, secret).then(({data}) => refineData(data, secret));
}

function getDetailedPlansFromPlans(
    planDirPath,
    detailedPlanDirPath,
    filterList,
    secret,
    ...
    let list = [];
    const files = fs.readdirSync(planDirPath).filter(file => {
        return filterFile(file, filterList);
    });
...
function saveDetailedPlan(id, detailedPlan, detailedPlanDirPath) {
    ...
    fs.writeFile(filePath, JSON.stringify(detailedPlan), err => {
        console.log(err);
    });
}
